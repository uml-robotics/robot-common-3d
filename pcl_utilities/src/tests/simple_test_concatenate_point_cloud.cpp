 ...
 *
 * Usage:
 *    `ros2 launch pcl_utilities test_concatenate_point_cloud.xml
 * point_cloud_topic:=<POINT_CLOUD_TOPIC>`
 */
#include <chrono>   // std::chrono::seconds
#include <cstddef>  // size_t
#include <cstdint>  // int64_t
#include <deque>  // std::deque<T, Allocator>
#include <ios>  // std::fixed, std::setprecision
#include <memory>  // std::make_shared<T, Args...>
...
{
private:
  rclcpp::Client<PCLConcatenatePointCloud>::SharedPtr concatenate_point_cloud_client_;
...
public:
  ...
  : rclcpp::Node("simple_test_concatenate_point_cloud")
  {
    ...
    num_point_clouds_ = narrowing_cast<size_t>(
      declare_parameter<int64_t>("num_point_clouds", std::move(param_constraints)));

    concatenate_point_cloud_client_ = create_client<PCLConcatenatePointCloud>(client_topic);

    output_publisher_ =
      create_publisher<PointCloud2>("concatenate_point_cloud/cloud_concatenated", 1);
  ...
  void process_point_cloud(PointCloud2 & point_cloud)
  {
    ...
    auto response_future = concatenate_point_cloud_client_->async_send_request(request);
    ...
    PCLConcatenatePointCloud::Response::SharedPtr response{response_future.get()};
