 ...
 * The point cloud is passed to the service, where all points outside of a
 * bounding box are removed. The bounds for the passthrough filter are
 * controlled via ros parameters, these parameters default to the shape of 1x1x1
 * meter unit-cube centered at (0, 0, 0).
 *
 * NOTES:
 * - That all of these parameters must produce a volume with a positive,
 * non-zero size.
 ...
 *
 * Usage:
 *    `ros2 launch pcl_utilities test_passthrough_filter_service.xml
...
{
private:
  rclcpp::Client<PCLPassthroughFilter>::SharedPtr passthrough_filter_client_;
...
public:
  ...
  : rclcpp::Node("simple_test_passthrough_filter")
  {
    ...
    passthrough_filter_client_ = create_client<PCLPassthroughFilter>(client_topic);

    output_publisher_ =
      create_publisher<PointCloud2>("test_passthrough_filter_service/cloud_filtered", 1);
  }
  ...
  void process_point_cloud(PointCloud2 && point_cloud)
  {
    ...
    auto response_future = passthrough_filter_client_->async_send_request(request);
    ...
    PCLPassthroughFilter::Response::SharedPtr response{response_future.get()};
